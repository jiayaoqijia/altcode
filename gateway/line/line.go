...
func NewLINEChannel(cfg config.LINEConfig, messageBus *gateway.MessageBus) (*LINEChannel, error) {
	if cfg.ChannelSecret == "" || cfg.ChannelAccessToken == "" {
		return nil, fmt.Errorf("line channel_secret and channel_access_token are required")
	}

	base := gateway.NewBaseChannel("line", cfg, messageBus, cfg.AllowFrom,
		gateway.WithMaxMessageLength(5000),
...
func (c *LINEChannel) processEvent(event lineEvent) {
	...
	scope := gateway.BuildMediaScope("line", chatID, msg.ID)
...
func (c *LINEChannel) Send(ctx context.Context, msg gateway.OutboundMessage) error {
	if !c.IsRunning() {
...
func (c *LINEChannel) SendMedia(ctx context.Context, msg gateway.OutboundMediaMessage) error {
	if !c.IsRunning() {
		return gateway.ErrNotRunning
	}
...
// StartTyping implements gateway.TypingCapable using LINE's loading animation.
//
...
func (c *LINEChannel) callAPI(ctx context.Context, endpoint string, payload any) error {
	...
	if err != nil {
		return gateway.ClassifyNetError(err)
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusOK {
		...
		if err != nil {
			return gateway.ClassifySendError(resp.StatusCode, fmt.Errorf("reading LINE API error response: %w", err))
		}
		return gateway.ClassifySendError(resp.StatusCode, fmt.Errorf("LINE API error: %s", string(respBody)))
	}
