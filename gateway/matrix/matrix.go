...
const (
	...
	matrixMediaTempDirName = "ottie_media"
)
...
func NewMatrixChannel(cfg config.MatrixConfig, messageBus *gateway.MessageBus) (*MatrixChannel, error) {
...
func (c *MatrixChannel) Send(ctx context.Context, msg gateway.OutboundMessage) error {
	if !c.IsRunning() {
	...
	if roomID == "" {
		return fmt.Errorf("matrix room ID is empty: %w", gateway.ErrSendFailed)
	}
	...
	if err != nil {
		return fmt.Errorf("matrix send: %w", gateway.ErrTemporary)
	}
	return nil
}
...
// SendMedia implements gateway.MediaSender.
func (c *MatrixChannel) SendMedia(ctx context.Context, msg gateway.OutboundMediaMessage) error {
	if !c.IsRunning() {
		return gateway.ErrNotRunning
	}
	sendCtx := ctx
	...
	for _, part := range msg.Parts {
		...
		if err != nil {
			...
			return fmt.Errorf("matrix upload media: %w", gateway.ErrTemporary)
		}
		...
		if _, err := c.client.SendMessageEvent(sendCtx, roomID, event.EventMessage, content); err != nil {
			...
			return fmt.Errorf("matrix send media: %w", gateway.ErrTemporary)
		}
	}
...
// StartTyping implements gateway.TypingCapable.
...
// SendPlaceholder implements gateway.PlaceholderCapable.
func (c *MatrixChannel) SendPlaceholder(ctx context.Context, chatID string) (string, error) {
	...
	if text == "" {
		text = gateway.RandomThinkingPhrase()
	}
...
// EditMessage implements gateway.MessageEditor.
...
func (c *MatrixChannel) handleMessageEvent(ctx context.Context, evt *event.Event) {
	...
	scope := gateway.BuildMediaScope("matrix", roomID, evt.ID.String())
	...
	c.HandleMessage(
		c.baseContext(),
		gateway.Peer{Kind: peerKind, ID: peerID},
		evt.ID.String(),
		senderID,
		roomID,
		content,
...
func localpartMentionRegexp(localpart string) *regexp.Regexp {
	...
	//   "@ottie" and "@ottie:matrix.org" should match,
	//   "test@example.com" and "helloottieworld" should not.
