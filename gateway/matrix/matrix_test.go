...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("ottie")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@ottie hello", want: true},
		{text: "hi @ottie:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bottie\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@ottie:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@ottie:matrix.org hello", want: "hello"},
		{in: "@ottie, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@ottie:matrix.org")},
				},
			},
			want: true,
		},
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@ottie:matrix.org">Ottie</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40ottie%3Amatrix.org">Ottie</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@ottie:matrix.test"), "")
	if err != nil {
