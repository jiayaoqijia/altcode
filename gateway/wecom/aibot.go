...
func NewWeComAIBotChannel(
	...
	base := gateway.NewBaseChannel("wecom_aibot", cfg, messageBus, cfg.AllowFrom,
		gateway.WithMaxMessageLength(2048),
...
func (c *WeComAIBotChannel) Send(ctx context.Context, msg gateway.OutboundMessage) error {
	if !c.IsRunning() {
	...
	if streamClosed {
		...
		if responseURL != "" {
			if err := c.sendViaResponseURL(responseURL, msg.Content); err != nil {
				...
				c.removeTask(task)
				return fmt.Errorf("response_url delivery failed: %w", gateway.ErrSendFailed)
			}
		} else {
...
func (c *WeComAIBotChannel) handleTextMessage(
	...
	go func() {
		...
		peer := gateway.Peer{Kind: peerKind, ID: chatID}
...
// Returned errors are wrapped with gateway.ErrRateLimit, gateway.ErrTemporary,
// or gateway.ErrSendFailed so the manager can apply the right retry policy.
func (c *WeComAIBotChannel) sendViaResponseURL(responseURL, content string) error {
	...
	if err != nil {
		return fmt.Errorf("post to response_url failed: %w: %w", gateway.ErrTemporary, err)
	}
	...
	if err != nil {
		return fmt.Errorf("reading response_url body: %w: %w", gateway.ErrTemporary, err)
	}
	switch {
	case resp.StatusCode == http.StatusTooManyRequests:
		return fmt.Errorf("response_url rate limited (%d): %s: %w",
			resp.StatusCode, respBody, gateway.ErrRateLimit)
	case resp.StatusCode >= 500:
		return fmt.Errorf("response_url server error (%d): %s: %w",
			resp.StatusCode, respBody, gateway.ErrTemporary)
	default:
		return fmt.Errorf("response_url returned %d: %s: %w",
			resp.StatusCode, respBody, gateway.ErrSendFailed)
	}
}
