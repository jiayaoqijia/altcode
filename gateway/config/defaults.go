...
// DefaultConfig returns the default configuration for Ottie.
func DefaultConfig() *Config {
	// Determine the base path for the workspace.
	// Priority: $OTTIE_HOME > ~/.ottie
	var homePath string
	if ottieHome := os.Getenv("OTTIE_HOME"); ottieHome != "" {
		homePath = ottieHome
	} else {
		userHome, _ := os.UserHomeDir()
		homePath = filepath.Join(userHome, ".ottie")
	}
	...
	return &Config{
		...
		Tools: ToolsConfig{
			...
			Web: WebToolsConfig{
				...
				Proxy:           "",
				FetchLimitBytes: 100 * 1024 * 1024, // 100MB
				Brave: BraveConfig{
					Enabled:    false,
					APIKey:     "",
					APIKeys:    nil,
			...
			Skills: SkillsToolsConfig{
				...
				Registries: SkillsRegistriesConfig{
					...
					LocalIndex: LocalIndexRegistryConfig{
						Enabled:       true,
						SyncOnStartup: true,
					},
				},
			...
			RenderHTML: RenderHTMLToolConfig{
