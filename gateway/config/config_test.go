...
func TestDefaultConfig_ExecAllowRemoteDisabled(t *testing.T) {
	...
	if cfg.Tools.Exec.AllowRemote {
		t.Fatal("DefaultConfig().Tools.Exec.AllowRemote should be false (secure default)")
	}
}
...
func TestLoadConfig_ExecAllowRemoteDefaultsFalseWhenUnset(t *testing.T) {
	...
	if cfg.Tools.Exec.AllowRemote {
		t.Fatal("tools.exec.allow_remote should default to false (secure default) when unset in config file")
	}
}
...
func TestDefaultConfig_SummarizationThresholds(t *testing.T) {
	...
	if cfg.Agents.Defaults.SummarizeMessageThreshold != 50 {
		t.Errorf("SummarizeMessageThreshold = %d, want 50", cfg.Agents.Defaults.SummarizeMessageThreshold)
	}
	if cfg.Agents.Defaults.SummarizeTokenPercent != 85 {
		t.Errorf("SummarizeTokenPercent = %d, want 85", cfg.Agents.Defaults.SummarizeTokenPercent)
	}
}
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join("/tmp/home", ".ottie", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithOttieHome(t *testing.T) {
	t.Setenv("OTTIE_HOME", "/custom/ottie/home")

	cfg := DefaultConfig()
	want := "/custom/ottie/home/workspace"

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with OTTIE_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
