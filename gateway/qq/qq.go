...
func NewQQChannel(cfg config.QQConfig, messageBus *gateway.MessageBus) (*QQChannel, error) {
	base := gateway.NewBaseChannel("qq", cfg, messageBus, cfg.AllowFrom,
		gateway.WithMaxMessageLength(cfg.MaxMessageLength),
...
func (c *QQChannel) Send(ctx context.Context, msg gateway.OutboundMessage) error {
	if !c.IsRunning() {
...
func (c *QQChannel) SendMedia(ctx context.Context, msg gateway.OutboundMediaMessage) error {
	if !c.IsRunning() {
		return gateway.ErrNotRunning
	}
	...
	for _, part := range msg.Parts {
		...
		if sendErr != nil {
			...
			return fmt.Errorf("qq send media: %w", gateway.ErrTemporary)
		}
	}
...
func (c *QQChannel) handleC2CMessage() event.C2CMessageEventHandler {
	return func(event *dto.WSPayload, data *dto.WSC2CMessageData) error {
		...
		c.HandleMessage(c.ctx,
			gateway.Peer{Kind: "direct", ID: senderID},
			data.ID,
			senderID,
			senderID,
			content,
...
func (c *QQChannel) handleGroupATMessage() event.GroupATMessageEventHandler {
	return func(event *dto.WSPayload, data *dto.WSGroupATMessageData) error {
		...
		c.HandleMessage(c.ctx,
			gateway.Peer{Kind: "group", ID: data.GroupID},
			data.ID,
			senderID,
			data.GroupID,
			content,
