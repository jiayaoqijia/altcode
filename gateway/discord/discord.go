...
func NewDiscordChannel(cfg config.DiscordConfig, bus *gateway.MessageBus) (*DiscordChannel, error) {
	...
	base := gateway.NewBaseChannel("discord", cfg, bus, cfg.AllowFrom,
		gateway.WithMaxMessageLength(2000),
...
func (c *DiscordChannel) Send(ctx context.Context, msg gateway.OutboundMessage) error {
	if !c.IsRunning() {
...
func (c *DiscordChannel) SendMedia(ctx context.Context, msg gateway.OutboundMediaMessage) error {
	if !c.IsRunning() {
		return gateway.ErrNotRunning
	}
	...
	case err := <-done:
		...
		if err != nil {
			return fmt.Errorf("discord send media: %w", gateway.ErrTemporary)
		}
		return nil
	case <-sendCtx.Done():
		// Close all file readers
...
// response via EditMessage (gateway.MessageEditor).
...
func (c *DiscordChannel) handleMessage(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	scope := gateway.BuildMediaScope("discord", m.ChannelID, m.ID)
	...
	peer := gateway.Peer{Kind: peerKind, ID: peerID}
