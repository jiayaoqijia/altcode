...
func NewFeishuChannel(cfg config.FeishuConfig, bus *gateway.MessageBus) (*FeishuChannel, error) {
	base := gateway.NewBaseChannel("feishu", cfg, bus, cfg.AllowFrom,
...
func (c *FeishuChannel) Send(ctx context.Context, msg gateway.OutboundMessage) error {
	if !c.IsRunning() {
...
func (c *FeishuChannel) SendMedia(ctx context.Context, msg gateway.OutboundMediaMessage) error {
	if !c.IsRunning() {
		return gateway.ErrNotRunning
	}
...
func (c *FeishuChannel) sendMediaPart(
	...
	if err != nil {
		...
		return fmt.Errorf("feishu send media: %w", gateway.ErrTemporary)
	}
	return nil
}
...
func (c *FeishuChannel) downloadInboundMedia(
	...
	var refs []string
	scope := gateway.BuildMediaScope("feishu", chatID, messageID)
...
func (c *FeishuChannel) downloadResource(
	...
	// Write to the shared ottie_media directory using a unique name to avoid collisions.
	mediaDir := filepath.Join(os.TempDir(), "ottie_media")
...
func (c *FeishuChannel) sendCard(ctx context.Context, chatID, cardContent string) error {
	...
	resp, err := c.client.Im.V1.Message.Create(ctx, req)
	if err != nil {
		return fmt.Errorf("feishu send card: %w", gateway.ErrTemporary)
	}

	if !resp.Success() {
		return fmt.Errorf("feishu api error (code=%d msg=%s): %w", resp.Code, resp.Msg, gateway.ErrTemporary)
	}
