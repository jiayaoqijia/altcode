...
**Trigger:** Only run this checklist if the diff touches frontend files. Check for frontend file extensions:

```bash
FRONTEND_FILES=$(git diff --name-only <base> -- '*.tsx' '*.jsx' '*.css' '*.scss' '*.vue' '*.svelte' '*.html' 2>/dev/null | head -5)
```

If no frontend files in the diff, skip the entire design review silently.
