...
CONTEXT_DIR="${CLAUDE_PLUGIN_DATA:-$HOME/.claude/data}"
PROJECT_HISTORY="$CONTEXT_DIR/projects/$REMOTE_SLUG/greptile-history.md"
```
...
REMOTE_SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
CONTEXT_DIR="${CLAUDE_PLUGIN_DATA:-$HOME/.claude/data}"
mkdir -p "$CONTEXT_DIR/projects/$REMOTE_SLUG"
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `$CONTEXT_DIR/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `$CONTEXT_DIR/greptile-history.md` (global aggregate)
