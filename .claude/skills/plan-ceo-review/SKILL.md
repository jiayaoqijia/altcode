...
DESIGN=$(ls -t .context/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t .context/*-design-*.md 2>/dev/null | head -1)
...
```bash
mkdir -p .context/ceo-plans/archive
# For each stale plan: mv .context/ceo-plans/{old-plan}.md .context/ceo-plans/archive/
```

Write to `.context/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```
NOTE: These represent human-team implementation hours. With AI-assisted,
...
* **Effort estimate:** S/M/L/XL (human team) → with AI-assisted: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
- **B)** Keep in `.context/` only (local, personal reference)
- **C)** Skip
